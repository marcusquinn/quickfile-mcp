...
describe("Tool Utilities", () => {
  describe("successResult", () => {
    ...
    it("should handle array data", () => {
      ...
      // Arrays are wrapped in { data: [...] } by sanitization layer
      expect(JSON.parse(result.content[0].text)).toEqual({ data: data });
    });

    it("should handle null data", () => {
      const result = successResult(null);

      // Null is wrapped in { data: null } by sanitization layer
      expect(JSON.parse(result.content[0].text)).toEqual({ data: null });
    });
  ...
  describe("handleToolError", () => {
    it("should handle QuickFileApiError with code and message", () => {
      ...
      expect(result.content[0].text).toBe(
        "QuickFile API Error [CLIENT_404]: Client not found",
      );
    });
